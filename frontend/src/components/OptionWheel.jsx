...
export const OptionWheel = ({
  ...
  const getRemPx = () => (typeof window !== 'undefined' ? parseFloat(getComputedStyle(document.documentElement).fontSize) || 16 : 16);
  ...
  cfgRef.current = {
    count: items.length,
    items,
    rowH: Math.max(fontSize * spacing * remPx, 45),
    curve,
    tilt,
    blur,
    fade,
  ...
  // Lay every option out along the curve based on distance from current position
  ...
  // Layout update after DOM paint
  useEffect(() => {
    renderLayout(posRef.current);
    const id1 = requestAnimationFrame(() => renderLayout(posRef.current));
    const id2 = setTimeout(() => renderLayout(posRef.current), 50);
    return () => {
      cancelAnimationFrame(id1);
      clearTimeout(id2);
    };
  }, [items, fontSize, spacing, curve, tilt, blur, fade, minOpacity, side, renderLayout]);
  ...
  // Sync when defaultSelected changes externally
  useEffect(() => {
    if (defaultSelected !== selectedRef.current) {
      ...
      renderLayout(defaultSelected);
      startLoop();
    }
  }, [defaultSelected, applyTarget, renderLayout, startLoop]);
      ...
      {items.map((label, index) => (
        <div
          ...
          ref={(el) => {
            ...
            if (el) renderLayout(posRef.current);
          }}
          role="option"
